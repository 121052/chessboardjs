...
function validFen(fen) {
    ...
    for (var i = 0; i < 8; i++) {
        if (chunks[i] === '' ||
        chunks[i].length > 8 ||
        chunks[i].search(/[^kqrbnpKQRNBPX1-8]/) !== -1) {
            return false;
        }
    }
    return true;
...
function validPositionObject(pos) {
    ...
    for (var i in pos) {
        ...
        var isValidSquare = (validSquare(i) === true);
        var isValidPieceCode = (validPieceCode(pos[i]) === true);

        if (!isValidSquare || !isValidPieceCode) {
            return false;
        }
    }
    return true;
    ...
    for (var i = 0; i < 8; i++) {
        ...
        for (var j = 0; j < row.length; j++) {
            ...
            // piece
                ...
                position[square] = (row[j] === 'X')? row[j] :fenToPieceCode(row[j]);
                colIndex++;
            }
        }
...
window['ChessBoard'] = window['ChessBoard'] || function(containerElOrId, cfg) {
    ...
    var MINIMUM_JQUERY_VERSION = '1.7.0',
        START_FEN = (cfg.darkMode === true)?
            'XXXXXXXX/XXXXXXXX/XXXXXXXX/XXXXXXXX/8/8/PPPPPPPP/RNBQKBNR':
    ...
    function checkDeps() {
        ...
        var JSONHasStringify = (typeof JSON.stringify !== 'function');
        var JSONHasParse = (typeof JSON.parse !== 'function');

        if (! window.JSON || JSONHasStringify || JSONHasParse) {
        ...
        var JQUERYExists = (typeof window.$ && $.fn && $.fn.jquery);
        if (! (JQUERYExists && compareSemVer($.fn.jquery, MINIMUM_JQUERY_VERSION) === true)) {
    ...
    function doAnimations(a, oldPos, newPos) {
        ...
        for (var i = 0; i < a.length; i++) {
            ...
            // move a piece
            if (a[i].type === 'move') {
                animateSquareToSquare(a[i].source, a[i].destination,
                                      a[i].piece, onFinish);
            }
        }
    }
    ...
    function isXYOnSquare(x, y) {
        for (var i in SQUARE_ELS_OFFSETS) {
            ...
            var isWithinX = (x >= s.left && x < s.left + SQUARE_SIZE);
            var isWithinY = (y >= s.top && y < s.top + SQUARE_SIZE);
    ...
    function snapbackDraggedPiece() {
        ...
        function complete() {
            ...
            var hasSnapbackEnd = (cfg.hasOwnProperty('onSnapbackEnd') === true);

            // run their onSnapbackEnd function
            if (hasSnapbackEnd && typeof cfg.onSnapbackEnd === 'function') {
    ...
    function beginDraggingPiece(source, piece, x, y) {
        ...
        var hasDragStart = (typeof cfg.onDragStart === 'function');
        var cancelDragStart = cfg.onDragStart(source, piece,
                                              deepCopy(CURRENT_POSITION),
                                              CURRENT_ORIENTATION);

        if (hasDragStart && cancelDragStart === false) {
            return;
        }
    ...
    function stopDraggedPiece(location) {
        ...
        var hasOnDrop = (cfg.hasOwnProperty('onDrop') === true);
        // run their onDrop function, which can potentially change the drop action
        if (hasOnDrop && typeof cfg.onDrop === 'function') {
            var newPosition = deepCopy(CURRENT_POSITION);
    ...
    */
    ...
    function mousedownSquare(e) {
        ...
        if (cfg.darkMode === true && CURRENT_POSITION[square] === 'X') return;

        // no piece on this square
        var hasPropertySquare = (CURRENT_POSITION.hasOwnProperty(square) !== true);
        if (validSquare(square) !== true || hasPropertySquare) {
            return;
        }
